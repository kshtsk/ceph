...
class TestMonitoring:
    ...
    # @patch("cephadm.serve.CephadmServe._refresh_hosts_and_daemons", lambda _: None)
    # @patch("cephadm.serve.CephadmServe._check_host", lambda _: None)
    @patch("cephadm.serve.CephadmServe._run_cephadm")
            ...
            with with_service(cephadm_module, MonitoringSpec('node-exporter')) as _, \
                ...
                _run_cephadm.assert_called_with(
                    ...
                    [
                        ...
                        '--meta-json',
                        ('{"service_name": "prometheus", "ports": [9095], "ip": "1.2.3.1", "deployed_by": [], "rank": null, '
                         '"rank_generation": null, "extra_container_args": null, "extra_entrypoint_args": null}'),
                        '--config-json', '-',
                        '--tcp-ports', '9095',
                        '--port-ips', '{"9095": "1.2.3.1"}'
                    ],
